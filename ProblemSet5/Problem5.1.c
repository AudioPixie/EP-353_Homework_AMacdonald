...
// clang Problem5.2.c -lsndfile && ./a.out
...
void process(float *inBuffer, float *outBuffer, sf_count_t bufferSize){
  for(sf_count_t n = 0; n < bufferSize; n++){
    ...
    if(outBuffer[n] < kThreshold && outBuffer[n] > 0){
      outBuffer[n] = 0; //Compress positive signals
    } else if(outBuffer[n] > -kThreshold && outBuffer[n] < 0){
      outBuffer[n] = 0; //Compress negative signals
    }
  }
}
